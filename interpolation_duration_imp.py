...
def creating_duration_healthybased(df_interp,saved_name, days=[7],hi=False):
    if hi==True:
    ...
    for f in days:
        if 'first_stuh_hg%d'%f not in df_interp.columns:
            ...
            df_interp = df_interp.groupby('patient_id').apply(lambda group: define_gh_imp(group, gap_healthy=f))
...
def main(argv):
    ...
    if 'hosp' in args.process:
        ...
        # df_interp = pd.read_csv('/home/csudre/MountedSpace/Covid/InterpolatedPosSHSymp.csv')
        # print(df_interp.shape)
        df_proc['hosp_valid'] = (df_proc['interval_days'] > df_proc['first_stuh_hg7']) * (
                    df_proc['location'] > 1)
        df_proc['hosp_check'] = df_proc.groupby('patient_id')['hosp_valid'].transform('max')
        df_proc['to_adjust'] = np.where(
            (df_proc['count_utoh_nans'] == 7) * (df_proc['interval_days'] > df_proc['first_stuh_hg7'])
            * (df_proc['interval_days'] < df_proc['last_stuh_hg7']) * (df_proc['hosp_check'] == 0) == 1, 1, 0)
        df_proc['max_adjust'] = df_proc.groupby('patient_id')['to_adjust'].transform('max')
        df_proc['max_adjust'] = df_proc.groupby('patient_id')['to_adjust'].transform('max')
        df_proc['day_adjust'] = np.where(df_proc['to_adjust'] == 1, df_proc['interval_days'], np.nan)
        df_proc['maxday_adjust'] = df_proc.groupby('patient_id')['day_adjust'].transform('max')
        df_proc['last_stuh_hg7_adj'] = np.where(df_proc['max_adjust'] == 1, df_proc['maxday_adjust'] - 7,
                                                  df_proc['last_stuh_hg7'])
        df_proc.to_csv(args.output)
